...
import { captureVisibleTab, getCurrentTabUrl, getCurrentTabId, getYoutubeVideoTime, sendToBackground, dataUrlToFile } from "./extension";
import { Brain, LogOut, Camera, Scissors, Globe, Loader2, ArrowLeft, CheckCircle2, AlertCircle } from "lucide-react";
...
export default function App() {
  ...
  const [loading, setLoading] = useState<"screenshot" | "screenshot_section" | "web" | null>(null);
  ...
  const handleScreenshot = async () => {
    ...
    try {
      ...
      if (tabId && url && url.includes("youtube.com/watch")) {
        const time = await getYoutubeVideoTime(tabId);
        if (time !== null) {
          url += (url.includes("?") ? "&" : "?") + `t=${time}s`;
        }
      }
      const dataUrl = await captureVisibleTab();
      const file = dataUrlToFile(dataUrl, "screenshot.png", "image/png");
      const body = buildCreateBody("image", title, "", url || "");
  ...
  const handleSectionCapture = async () => {
    ...
    if (loading === "screenshot_section") {
      try {
        const tabId = await getCurrentTabId();
        if (tabId != null) {
          await sendToBackground({ action: "cancelSectionCapture", tabId });
        }
      } catch (err) {
    ...
    try {
      ...
      const res = await sendToBackground({ action: "startSectionCapture", tabId, title, sourceUrl: url || undefined });
      if (res.ok) {
        // Only show success if it wasn't cancelled (res.ok is true for cancel too, but we might want to check res.cancelled)
        if (res.cancelled) {
          setLoading(null);
          return;
        }
          ...
          <div className="py-10 flex flex-col items-center text-center space-y-8">
            <div className="space-y-4">
              ...
              <p className="font-bold text-gray-500 text-xs uppercase tracking-widest leading-relaxed max-w-[280px]">
                Save screenshots and links to your second brain instantly.
              </p>
            </div>
          ...
          <>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <button
                ...
                disabled={!!loading && loading !== "screenshot_section"}
                className={`brut-card p-4 flex flex-col items-center gap-2 transition-colors group ${loading === "screenshot_section" ? "bg-rose-50 border-rose-500 shadow-[2px_2px_0px_0px_#f43f5e]" : "hover:bg-indigo-50"}`}
              >
                <div className={`w-10 h-10 border-2 border-black flex items-center justify-center rotate-[-2deg] group-hover:rotate-0 transition-transform ${loading === "screenshot_section" ? "bg-rose-200" : "bg-indigo-100"}`}>
                  {loading === "screenshot_section" ? <Loader2 className="animate-spin" /> : <Scissors size={20} />}
                </div>
                <span className="font-black uppercase text-[10px] tracking-widest">
                  {loading === "screenshot_section" ? "Cancel" : "Section"}
                </span>
              </button>
